...
function Item(props) {
    const { to,state, id,lefticonname,righticonname, iconcolor,text, onClick, className} = props; // Destructure props to extract specific values
    ...
    return (
        <Link to={to} state={{indexpage:state}}  id={id} variant='warning' onClick={onClick} className={`btn ${styles.item} ${_class}`} style={linkstyles}>
